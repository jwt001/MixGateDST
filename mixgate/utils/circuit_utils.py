...
def logic(gate_type, signals, gate_to_index):
    ...
    elif 'BUF' in gate_to_index.keys() and gate_type == gate_to_index['BUF']:  # BUF
        for s in signals:
            return s
    ...
    elif 'XOR' in gate_to_index.keys() and gate_type == gate_to_index['XOR'] and len(signals) == 2:  # XOR
        z_count = 0
        o_count = 0
        for s in signals:
            if s == 0:
    ...
    elif 'XOR' in gate_to_index.keys() and gate_type == gate_to_index['XOR'] and len(signals) == 3:  # XOR
        z_count = 0
        o_count = 0
        final_list = []
        final_list.append(signals[0])
        for s in signals[1:]:
            if s == 0:
                z_count = z_count + 1
            elif s == 1:
...
def add_node_index(data):
    ...
    for i, val in enumerate(data):
        ...
        if "INPUT" in val:
            node_name = val.split("(")[1].split(")")[0]#括号中的数字
        ...
        if ("= NAND" in val) or ("= NOR" in val) or ("= AND" in val) or ("= OR" in val) or (
                "= NOT" in val) or ("= XOR" in val) or ("= MAJ" in val) or ("= BUF" in val):
...
def feature_gen_connect(data, gate_to_index):
    ...
    for line in data:
        if 'INPUT(' in line:
            node_name = line.split("(")[-1].split(')')[0]
            new_node(name2idx, x_data, node_name, get_gate_type('INPUT', gate_to_index))#返回节点信息，存储到x_data中
            
        elif 'AND(' in line or 'NAND(' in line or 'OR(' in line or 'NOR(' in line \
                or 'NOT(' in line or 'XOR(' in line or 'BUF(' in line or 'MAJ(' in line:
            # print("line =", line)
            node_name = line.split(':')[-1].split('=')[0].replace(' ', '')
            gate_type = line.split('=')[-1].split('(')[0].replace(' ', '')
            new_node(name2idx, x_data, node_name, get_gate_type(gate_type, gate_to_index))
            # print("node_name =", name2idx)
...
def feature_gen_level(x_data, fanout_list, gate_to_index={'gnd': 999, 'vdd': 999}):
    ...
    for idx, x_data_info in enumerate(x_data):
        if x_data_info[1] == 0 or x_data_info[1] == 'PI':
            bfs_q.append(idx)
            x_data_level[idx] = 0
        #print("bfs_q =", bfs_q)#pi的list
    while len(bfs_q) > 0:
        idx = bfs_q[-1]
        bfs_q.pop()
        tmp_level = x_data_level[idx] + 1 #tmp_level = 1 right now
        for next_node in fanout_list[idx] :
            if x_data_level[next_node] < tmp_level:
                x_data_level[next_node] = tmp_level
                bfs_q.insert(0, next_node)
                # for i, ele in enumerate(fanout_list):
                #     print("idx =", i, "fanout =", ele)
                # print("bfs_q ", bfs_q)
                if x_data_level[next_node] > max_level:
                    max_level = x_data_level[next_node]
    level_list = []
...
def parse_bench(file, gate_to_index={'INPUT': 0, 'MAJ': 1, 'NOT': 2, 'XOR': 3}, MAX_LENGTH=-1):
    data = read_file(file)
    data, num_nodes, _ = add_node_index(data)#返回节点数量
...
def simulator_truth_table(x_data, PI_indexes, level_list, fanin_list, gate_to_index):
    ...
    for pattern_idx in range(no_of_patterns):#单个输入的大小
        input_vector = dec2list(pattern_idx, len(PI_indexes))#从小到大的排列，将10进制转为2进制计算
        #input_vector 为二进制列表，值的大小用于遍历每个门
        state = [-1] * len(x_data)#有多少个门

        for k, pi_idx in enumerate(PI_indexes):#PI_index存储了pi节点的下标
            state[pi_idx] = input_vector[k]#PI的输入应该是多少

        for level in range(1, len(level_list), 1):#所有层级
            for node_idx in level_list[level]:#第1个层级的第0个点index为5
                source_signals = []
                for pre_idx in fanin_list[node_idx]:#层级为1，index为5的点的fanin
                    source_signals.append(state[pre_idx])#src所有的输入
                if len(source_signals) > 0:
                    gate_type = x_data[node_idx][1]#找出index为5的值gate type
                    res = logic(gate_type, source_signals, gate_to_index)#计算
                    state[node_idx] = res

        for idx in range(len(x_data)):
            truth_table[idx].append(state[idx])
    
    return truth_table#输出每个节点对应的真值表信息
...
def simulator_truth_table_random(x_data, PI_indexes, level_list, fanin_list, gate_to_index, num_patterns=15000):
    ...
    for pattern_idx in range(num_patterns):#抽取15000种可能
        input_vector = random_pattern_generator(len(PI_indexes))
        state = [-1] * len(x_data)
